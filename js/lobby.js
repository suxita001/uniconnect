...
document.getElementById("btn-find-chat").addEventListener("click", async () => {
  ...
  try {
    ...
    // Note: avoid "!=" operator (requires composite index) — filter client-side instead
    ...
    // Filter out current user client-side (avoids needing composite index for !=)
    const candidates = waitingSnap.docs.filter(d => d.data().uid !== currentUser.uid);
    let matched = false;

    for (const partnerDoc of candidates.slice(0, 5)) {
      try {
